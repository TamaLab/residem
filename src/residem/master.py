...
def deg_convert(theta):
    ...
    """
    theta = math.fmod(math.fmod(theta, 360) + 360,
                      360) if theta < 0 else math.radians(theta)
    return theta
...
class DataInput:
    def __init__(self, reference_pdb, reference_mtz, Triggered_mtz, reference_label=None, triggered_label=None, fcalclabel=None,
        ...
        self.label_phase = self.map_obs_label(
            reference_label, triggered_label, fcalclabel)[2].split(",")

        self.high_resolution = high_resolution if high_resolution is not None else self.map_resolution()[
            1]
        self.low_resolution = low_resolution if low_resolution is not None else self.map_resolution()[
            0]
    ...
    def map_resolution(self):
        if self.reference_label is not None and self.triggered_label is not None:
            dark_dmax = round(
                self.dark_dict[self.reference_label].d_max_min()[0], 1)
            dark_dmin = round(
                self.dark_dict[self.reference_label].d_max_min()[1], 1)
            light_dmax = round(
                self.light_dict[self.triggered_label].d_max_min()[0], 1)
            light_dmin = round(
                self.light_dict[self.triggered_label].d_max_min()[1], 1)
    ...
    def map_obs_label(self, A, B, C):
        ...
        label_phase = [X for X in list(self.dark_col) if X == 'FC,PHIC' or X ==
                       'FC_ALL,PHIC_ALL' or X == 'FC,PHIFC' or X == "F-model,PHIF-model" or X == C][0]
...
def write_phil(output_path, refernce_pdb, refernce_mtz, triggered_mtz, reference_label, triggred_label, high_resolution, low_resolution,
    ...
    params = iotbx.phil.parse(
        fo_minus_fo_master_params_st, process_includes=True)
    ...
    modified_phil.show(
        out=open(f"{output_path}/input.phil", "w"), attributes_level=2)
...
The input file for the current calculation is written as {f"{name_path}/input.phil"} which can be used to run to recalculation.""")
    ...
    if scaling_method == "cctbx":
        ...
        result_mtz_path = os.path.realpath(
            f"%s/Light-dark-scaled.mtz" % name_path)
    ...
    if weight == "ren":
        map_model = map_model_manager(
            map_manager=Map_file.FOFODF_K.as_map_manager(), model=Map_file.model)

        to_identify = MapPeakSelection_coff(
            map_model, Map_file.high_resolution, sigma, name=weight, path=name_path, write=True)
        ...
        map_model = map_model_manager(
            map_manager=Map_file.FOFODF_Q.as_map_manager(), model=Map_file.model)
        ...
        map_model = map_model_manager(
            map_manager=Map_file.FOFODF_H.as_map_manager(), model=Map_file.model)
        ...
        map_model = map_model_manager(
            map_manager=Map_file.FOFODF.as_map_manager(), model=Map_file.model)
...
{f"{name_path}/map_dump_default/chain_A_U/Residual_peak_height_max_chain_A_U.pdf"}""")
    logger.info(f"""{"#" * 20}""")
    logger.info(
        f"""Time taken for over all calculation is {datetime.now() - start_calculation}""")
...
def run_all():
    ...
    parser.add_argument("-p", "--phil", type=str,
                        help="process parameter file")
    parser.add_argument("-d", "--default", action='store_true',
                        help="write default parameter file")
    parser.add_argument("-r", "--ref", type=str, help="reference pdb file")
    parser.add_argument("-m", "--mtz", type=str,
                        help="reference hkl file in mtz format")
    parser.add_argument("-t", "--tmtz", type=str,
                        help="triggered hkl file in mtz format")
    ...
    parser.add_argument("-fl", "--fcalclabel", action='store',
                        help="label of the refined f_calc file example FC,PHIC")

    parser.add_argument("-s", "--sigma", type=float,
                        help="sigma cutoff for map processing")
    parser.add_argument("-w", "--weight", type=str,
                        help="weight for choosing the map calculation")
    parser.add_argument("-ws", "--alpha", type=float,
                        help="value of alpha in Hekstra weight, default 0.05 ")
    ...
    if args.default:
        ...
        fo_minus_fo_master_params_st = " ".join(
            open(args.phil, "r").readlines())
        params = iotbx.phil.parse(
            fo_minus_fo_master_params_st, process_includes=True).extract()
        ...
        scale = params.scale if params.scale in [
            "iso", "aniso", "no"] else "aniso"
        ...
        run(reference_pdb, reference_mtz, Triggered_mtz, reference_label, triggered_label, high_resolution, low_resolution,
            phil_sigma, phil_weight, scale, atom_profile, phil_alpha, scaling_method, ccp4_scaling_method, fcalclabel)
    ...
    if len(sys.argv) == 1:
        ...
        scaling_method = args.scaling_method if args.scaling_method in [
            "cctbx", "ccp4"] else "cctbx"
        ccp4_scaling_method = args.ccp4_scaling_method if args.scaling_method == "ccp4" and args.ccp4_scaling_method in [
            "linear"] else "linear"
        if scaling_method == "ccp4":
            scale = "no"
